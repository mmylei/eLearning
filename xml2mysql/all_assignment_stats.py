...
for term_key in terms:
    ...
    create_grades_table(conn, term + "_all_assignment_stats")
    cursor = conn.cursor()
    ...
    for module in modules[term_key]:
        ...
        if module_name == 'Exam':
            ...
            problem_types = [module_name, module_name.replace('M', 'L'), module_name.replace('M', 'Q'),
                             module_name.replace('M', 'T'), module_name.replace('M', 'P')]
        else:
            problem_types = [module_name]
        for problem_type in problem_types:
            # get all problems under this module
            cursor.execute("SELECT element_id from " + terms[term_key] + term + "_element where aggregated_category = \""
            ...
            for row in result:
                ...
                cursor.execute(
                    "INSERT INTO " + term + "_all_assignment_stats VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
